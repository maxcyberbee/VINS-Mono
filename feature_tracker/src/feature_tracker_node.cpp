...
public:
    ...
    vector<float> r_err;
    queue<sensor_msgs::msg::Image> img_buf;
    ...
    {
        auto pub_img = this->create_publisher<std_msgs::msg::String>("chatter", 1000);
        auto pub_match = this->create_publisher<sensor_msgs::msg::Image>("feature_img", 1000);
        auto pub_equalize = this->create_publisher<sensor_msgs::msg::Image>("/cam0/image/equalize", 1000);
        auto pub_restart = this->create_publisher<std_msgs::msg::Bool>("restart", 1000);
        this->create_subscription<sensor_msgs::msg::Image>(IMAGE_TOPIC, 100, std::bind(&MinimalSubscriber::img_callback, this, _1));
    }
...
private:
    ...
    void img_callback(const sensor_msgs::msg::Image &img_msg) 
    {
        ...
        {
            // RCLCPP_DEBUG("processing camera %d", i);
            if (i != 1 || !STEREO_TRACK)
            {
        ...
        {
            pub_count++;
            sensor_msgs::msg::PointCloud *feature_points = new sensor_msgs::msg::PointCloud;
            ...
            // RCLCPP_DEBUG("publish %f, at %f", feature_points->header.stamp.toSec(), ros::Time::now().toSec());
...
int main(int argc, char **argv)
{
    ...
    if (FISHEYE)
    {
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            {
                // RCLCPP_INFO("load mask fail");
                // ROS_BREAK();
            }
            else
            {
            }
            // RCLCPP_INFO("load mask success");
        }
    }
