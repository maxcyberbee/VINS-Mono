...
void printStatistics(const Estimator &estimator, double t,rclcpp::Logger logger)
{
  ...
  //  RCLCPP_INFO(logger,"position: %f, %f, %f\r", estimator.Ps[WINDOW_SIZE].x(), estimator.Ps[WINDOW_SIZE].y(), estimator.Ps[WINDOW_SIZE].z());
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        RCLCPP_DEBUG_STREAM(logger,"extirnsic tic: " << estimator.tic[i].transpose());
        RCLCPP_DEBUG_STREAM(logger,"extrinsic ric: " << Utility::R2ypr(estimator.ric[i]).transpose());
    ...
    RCLCPP_DEBUG(logger,"average of time %f ms", sum_of_time / sum_of_calculation);

    sum_of_path += (estimator.Ps[WINDOW_SIZE] - last_path).norm();
    last_path = estimator.Ps[WINDOW_SIZE];
    RCLCPP_DEBUG(logger,"sum of path %f", sum_of_path);
    if (ESTIMATE_TD)
        RCLCPP_INFO(logger,"td %f", estimator.td);
}
